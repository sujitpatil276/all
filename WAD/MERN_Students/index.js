...
const db_connection = "mongodb+srv://fivestarreviewuser:balaji212@fivestarmoviereview.phe0ehu.mongodb.net/w1?retryWrites=true&w=majority"
const port = 2324
...
app.get("/displayAllStudentsInTable", async function (request, response) {
    ...
    // creating headers 
    // **BackTick (`) is used for creating multiline string**
    html += `<tr>
        <th>Name</th>
        <th>Roll_No</th>
        <th>WAD_Marks</th>
...
app.delete("/delete/:rollno", async function (request, response) {
    const studentNo = request.params.rollno
    const deletedStudent = await StudentMarks.findOneAndDelete({ Roll_No: studentNo })
